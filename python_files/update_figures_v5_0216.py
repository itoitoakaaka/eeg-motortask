...
# Settings
excel_path = '/Users/itoakane/Research/SEP_processed/measurement2 2.xlsx'
...
# Columns in measurement2 2.xlsx are already 'ppi30', 'ppi100', 'ppi30_change', 'ppi100_change'
# Standardize on these names

# Include all subjects except id001
df_ana = df[df['file_id'].astype(str).str.contains('id0') & ~df['file_id'].astype(str).str.contains('id001')].copy()
...
def plot_bar_land_water(ax, data_l, data_w, title, ylabel):
    ...
    x = [0, 1]
    means = [np.mean(data_l), np.mean(data_w)] if len(data_l)>0 and len(data_w)>0 else [0, 0]
    sems = [stats.sem(data_l), stats.sem(data_w)] if len(data_l)>0 and len(data_w)>0 else [0, 0]
...
def plot_relation(ax, df_all, x_col, y_col, x_l, y_l, x_w, y_w, xlabel, ylabel, title, method='rmcorr', common_ylim=None):
    ...
    try:
        ...
        if method == 'rmcorr' and len(df_clean) > 2 and df_clean['sid'].nunique() > 1:
            ...
            # Add overall regression line if RMCorr is significant
            if p_all < 0.05:
                # Use common slope from RMCorr model
                # y = beta_0 + beta_sid * sid + beta_x * x
                # For visualization, we use the average intercept across sids
                ...
                p_overall = [avg_intercept, beta_x] # [c, m] for Polynomial
...
# Fig 2: Pre-Post (4 groups + Hatching + Scale Optimized)
...
for i, (v, vlabel) in enumerate(zip(target_vars, vlabels)):
    ...
    errs = [stats.sem(l_pre) if len(l_pre)>0 else 0, stats.sem(l_post) if len(l_post)>0 else 0, 
            stats.sem(w_pre) if len(w_pre)>0 else 0, stats.sem(w_post) if len(w_post)>0 else 0]
...
print(f"Update of {output_dir} using measurement2 2.xlsx with PPI naming complete.")
