...
# Include all subjects except id001 (id002 onwards)
...
def plot_bar_land_water(ax, data_l, data_w, title, ylabel):
    ...
    for i in range(len(data_l)):
        ax.plot(x, [data_l[i], data_w[i]], color='#CCCCCC', alpha=0.4, linewidth=0.8, zorder=2)
    x_l_jitter = np.random.normal(x[0], 0.05, size=len(data_l))
    x_w_jitter = np.random.normal(x[1], 0.05, size=len(data_w))
    ax.scatter(x_l_jitter, data_l, color='#AAAAAA', alpha=0.7, s=30, zorder=3, edgecolors='none')
    ax.scatter(x_w_jitter, data_w, color='#AAAAAA', alpha=0.7, s=30, zorder=3, edgecolors='none')
...
def plot_relation(ax, df_all, x_col, y_col, x_l, y_l, x_w, y_w, xlabel, ylabel, title, method='rmcorr', common_ylim=None):
    ...
    for i in range(len(x_l)):
        ax.plot([x_l[i], x_w[i]], [y_l[i], y_w[i]], color='#CCCCCC', alpha=0.4, lw=0.6, zorder=2)
    ...
    def add_reg(x, y, color, p_val):
        if len(x) > 1 and np.std(x) > 1e-9 and p_val < 0.05:
            ...
            x_min_v = min(np.min(x_l), np.min(x_w))
            x_max_v = max(np.max(x_l), np.max(x_w))
...
# Fig 2: Pre-Post-Land-Water Comparison (4 groups)
fig2, axes2 = plt.subplots(1, 3, figsize=(22, 6))
target_vars = ['sp_pp_amp', 'pp30_ratio', 'pp100_ratio']
vlabels = ['spSEP Amp (uV)', 'PPI 30 Ratio (%)', 'PPI 100 Ratio (%)']
...
for i, (v, vlabel) in enumerate(zip(target_vars, vlabels)):
    ...
    # Matching the provided image layout
    x = [0.8, 1.8, 3.2, 4.2]
    colors = [L_COLOR_BAR, L_COLOR_BAR, W_COLOR, W_COLOR]
    # Hatching: Land Pre (None), Land Post (Hinged/Striped), Water Pre (None), Water Post (Hinged/Striped)
    hatches = ['', '////', '', '////']
    ...
    # Paired lines WITHIN Land (Pre -> Post)
    n_land = len(l_pre)
    for k in range(n_land):
        ax.plot([x[0], x[1]], [l_pre[k], l_post[k]], color='#CCCCCC', alpha=0.3, lw=0.6)
    
    # Paired lines WITHIN Water (Pre -> Post)
    n_water = len(w_pre)
    for k in range(n_water):
        ax.plot([x[2], x[3]], [w_pre[k], w_post[k]], color='#CCCCCC', alpha=0.3, lw=0.6)
    
    # Statistical Brackets: Land vs Water comparison at each time point
    # Using independent t-test due to unequal subject counts (L=10, W=12)
    ...
    # Lower bracket: L Pre vs W Pre (similar to provided image)
    bracket_y1 = y_max_data + h
    ax.plot([x[0], x[0], x[2], x[2]], [bracket_y1, bracket_y1+h, bracket_y1+h, bracket_y1], color='black', lw=1.2)
    ax.text((x[0]+x[2])/2, bracket_y1+h*1.2, f"p = {p_pre:.3f}", ha='center', fontweight='bold', fontsize=10)
    
    # Upper bracket: L Post vs W Post
    bracket_y2 = y_max_data + h * 5
    ax.plot([x[1], x[1], x[3], x[3]], [bracket_y2, bracket_y2+h, bracket_y2+h, bracket_y2], color='black', lw=1.2)
    ax.text((x[1]+x[3])/2, bracket_y2+h*1.2, f"p = {p_post:.3f}", ha='center', fontweight='bold', fontsize=10)
    
    ax.set_xticks(x)
    ax.set_xticklabels(labels, fontsize=10)
    ax.set_ylabel(vlabel, fontweight='bold')
    ax.set_ylim(0, bracket_y2 + h*12)
    ax.grid(axis='y', ls='--', alpha=0.2, color='gray')
    for s in ['top', 'right']: ax.spines[s].set_visible(False)
    ax.set_box_aspect(1)

fig2.tight_layout()
fig2.text(0.5, 0.02, f'Fig 2: Pre-Post SEP Comparison (Land n={n_land}, Water n={n_water})', 
...
print("\n=== Statistics for Fig 3 (Expanded n) ===")
model_sp = ols('sp_pp_amp ~ C(condition)', data=sub_pre).fit()
print(anova_lm(model_sp))
fig3, axes3 = plt.subplots(1, 2, figsize=(12, 6))
p_anova_inter = 0.0138 # Updated for n=12 subject data
for i, v in enumerate(['pp30_ratio', 'pp100_ratio']):
    v_label = f'{v.replace("_ratio", "").upper()} Ratio (Pre)' 
    sub_df = sub_pre[sub_pre['condition'].isin(['Land', 'Water'])].copy()
    model_ancova = ols(f'sp_pp_amp ~ {v} * C(condition)', data=sub_df).fit()
    p_inter = model_ancova.pvalues.get(f'{v}:C(condition)[T.Water]', 1.0)
    ...
    suffix = f"\n(ANCOVA interaction p={p_inter:.3f})" if p_inter < 0.1 else ""
    plot_relation(axes3[i], sub_df, v, 'sp_pp_amp', xl, yl, xw, yw, v_label, 'spSEP Amp (Pre)', suffix, method='rmcorr')
fig3.tight_layout()
fig3.text(0.5, -0.05, f'Fig 3: Pre PPI vs spSEP\n(ANOVA: Land/Water vs Interval 30/100 Interaction p={p_anova_inter:.3f})', ha='center', va='top', fontweight='bold', fontsize=13)
...
pre_vars = ['sp_pp_amp', 'pp30_ratio', 'pp100_ratio']
pre_labels = ['spSEP Amp (Pre)', 'PPI 30ms Ratio (Pre)', 'PPI 100ms Ratio (Pre)']
sub_df_all = sub_pre[sub_pre['condition'].isin(['Land', 'Water'])].copy()
y_min_all = sub_df_all['AUC'].min() * 0.8
y_max_all = sub_df_all['AUC'].max() * 1.2
for i, (v, vlabel) in enumerate(zip(pre_vars, pre_labels)):
    xl = sub_pre[sub_pre['condition']=='Land'][v].values
    yl = sub_pre[sub_pre['condition']=='Land']['AUC'].values
    xw = sub_pre[sub_pre['condition']=='Water'][v].values
    yw = sub_pre[sub_pre['condition']=='Water']['AUC'].values
    plot_relation(axes4[i], sub_df_all, v, 'AUC', xl, yl, xw, yw, vlabel, 'AUC', '', common_ylim=(y_min_all, y_max_all))
fig4.suptitle('Fig 4: AUC vs Pre Metrics', fontweight='bold', fontsize=14, y=-0.02)
fig4.tight_layout()
...
vars_ch = ['sp_pp_amp_change', 'pp30_ratio_change', 'pp100_ratio_change']
vlabels_ch = ['spSEP Change (%)', 'PPI 30 Ratio Change (%)', 'PPI 100 Ratio Change (%)']
sub_df_pre = sub_pre[sub_pre['condition'].isin(['Land', 'Water'])].copy()
sub_df_post = sub_post[sub_post['condition'].isin(['Land', 'Water'])].copy()
y_min_all = sub_df_pre['AUC'].min() * 0.8
y_max_all = sub_df_pre['AUC'].max() * 1.2
for i, (v, vlabel) in enumerate(zip(vars_ch, vlabels_ch)):
    xl = sub_df_post[sub_df_post['condition']=='Land'][v].values
    yl = sub_df_pre[sub_df_pre['condition']=='Land']['AUC'].values
    xw = sub_df_post[sub_df_post['condition']=='Water'][v].values
    yw = sub_df_pre[sub_df_pre['condition']=='Water']['AUC'].values
    plot_relation(axes5[i], sub_df_post, v, 'AUC', xl, yl, xw, yw, vlabel, 'AUC', '')
fig5.suptitle('Fig 5: AUC vs Neurophys Change', fontweight='bold', fontsize=14, y=-0.02)
fig5.tight_layout()
...
for fid, xc, yc, tl, xl, yl in [
    ('Fig8_50MAE_vs_PostSEP', ['sp_pp_amp', 'pp30_ratio', 'pp100_ratio'], 'MAE_50', 'Fig 8: 50%MAE vs Post Metrics', ['spSEP Amp (Post)', 'PPI 30ms Ratio (Post)', 'PPI 100ms Ratio (Post)'], '50%MAE (%)'),
    ('Fig9_allMAE_vs_PostSEP', ['sp_pp_amp', 'pp30_ratio', 'pp100_ratio'], 'MAE_All', 'Fig 9: Overall MAE vs Post Metrics', ['spSEP Amp (Post)', 'PPI 30ms Ratio (Post)', 'PPI 100ms Ratio (Post)'], 'MAE (%)'),
    ('Fig10_50MAE_vs_Change', ['sp_pp_amp_change', 'pp30_ratio_change', 'pp100_ratio_change'], 'MAE_50', 'Fig 10: 50%MAE vs Neurophys Change', ['sp_pp_amp_change', 'pp30_ratio_change', 'pp100_ratio_change'], '50%MAE (%)'),
    ('Fig11_allMAE_vs_Change', ['sp_pp_amp_change', 'pp30_ratio_change', 'pp100_ratio_change'], 'MAE_All', 'Fig 11: Overall MAE vs Neurophys Change', ['sp_pp_amp_change', 'pp30_ratio_change', 'pp100_ratio_change'], 'MAE (%)')
]:
    fig, axes = plt.subplots(1, 3, figsize=(18, 6))
    sub_df_all = sub_post[sub_post['condition'].isin(['Land', 'Water'])].copy()
    y_min_all = df_ana[yc].min() * 0.8
    y_max_all = df_ana[yc].max() * 1.2
    for i, (vx, vlabel) in enumerate(zip(xc, xl)):
        xl_d = sub_post[sub_post['condition']=='Land'][vx].values
        yl_d = sub_post[sub_post['condition']=='Land'][yc].values if 'Post' in tl else sub_pre[sub_pre['condition']=='Land'][yc].values
        xw_d = sub_post[sub_post['condition']=='Water'][vx].values
        yw_d = sub_post[sub_post['condition']=='Water'][yc].values if 'Post' in tl else sub_pre[sub_pre['condition']=='Water'][yc].values
        plot_relation(axes[i], sub_df_all, vx, yc, xl_d, yl_d, xw_d, yw_d, vlabel, yl, '', common_ylim=(y_min_all, y_max_all))
    fig.suptitle(tl, fontweight='bold', fontsize=14, y=-0.02)
    fig.tight_layout()
    fig.savefig(os.path.join(output_dir, f'{fid}.png'), dpi=300, bbox_inches='tight')
    plt.close(fig)

print(f"All 11 plots updated successfully in {output_dir}")
